...
// Función para enviar correos electrónicos con un archivo PDF adjunto
function enviarCorreoConPdf($correo, $pdfPath) {
    ...
    try {
        $mail->setFrom('tu_correo@gmail.com', 'HSE TF'); // Cambiar 'tu_correo@gmail.com' a tu dirección de correo
        $mail->addAddress($correo);
        $mail->isHTML(true);
        $mail->Subject = 'INSPECCION BOTIQUIN';
        $mail->Body = "Inspeccion Botiquin.";
        $mail->addAttachment($pdfPath, 'inspeccion_botiquin.pdf');
        $mail->send();
...
if (isset($_POST['btnEnviarCorreo'])) {
    ...
    // Generar el PDF
    $pdfPath = 'inspeccion_botiquin.pdf';
