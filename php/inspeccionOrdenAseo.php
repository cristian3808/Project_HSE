...
function enviarCorreoConPdf($correo, $pdfPath) {
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = 'INSPECCION ORDEN Y ASEO';
        $mail->Body = "Inspeccion orden y aseo.";
        $mail->addAttachment($pdfPath, 'inspeccion_orden_aseo.pdf');
        $mail->send();
...
if (isset($_POST['btnEnviarCorreo'])) {
    ...
    // Generar el PDF
    $pdfPath = 'inspeccion_orden_aseo.pdf';
