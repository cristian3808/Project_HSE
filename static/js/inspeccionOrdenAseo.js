...
const instalacionesInput = document.getElementById('instalaciones');
...
function showAlertIfMissingFields(event) {
    let missingFields = [];
    if (!instalacionesInput.value) missingFields.push("instalaciones");
    if (!areaInput.value) missingFields.push("area");
...
downloadButton.addEventListener('click', function() {
    ...
    html2canvas(formContent, { backgroundColor: null }).then(canvas => {
        ...
        // Guardar PDF
        pdf.save('inspeccion_orden_aseo.pdf');
